...
function gameInit() {
    startPieces.forEach(
        function(piece,index) {
            ...
            square.setAttribute("rowIndex",rowIndex);
            square.setAttribute("colIndex",colIndex);
            square.classList.add("Square");
            square.innerHTML = piece;
            if(piece == Pawn) square.firstChild.setAttribute('firstMove',true);
            square.classList.add(
                (rowIndex + colIndex) % 2 == 0 ? "BeigeColor" : "BrownColor"
            )
            
            if(rowIndex <= 1) {
                square.firstChild.classList.add("BlackPlayer")
                square.firstChild?.setAttribute('draggable',false);
            };
            if(rowIndex >= 6) {
                square.firstChild.classList.add("WhitePlayer")
    ...
    squares.forEach(function(square) {
        ...
        square.addEventListener('dragover',moveDrag);
        square.addEventListener('drop',stopDrag);
...
function clickHandle(e){
    if(!hasClicked) {
        ...
        validSquare = canMove(currentPiece);
        if(currentPiece.classList.contains(turn)) {
            for (const square of validSquare ) {
